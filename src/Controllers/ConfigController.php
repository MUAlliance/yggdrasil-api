...
class ConfigController extends Controller
{
    public function render()
    {
        ...
        $restoreAPIForm = Option::form('restore', trans('Yggdrasil::config.restore.title'), function (OptionForm $form) {
        ...
        $unionForm = Option::form('union', trans('Yggdrasil::config.union.title'), function (OptionForm $form) {
            ...
            /*
          	$form->checkbox('union_use_blacklist_locally', trans('Yggdrasil::config.union.local_blacklist.title'))
                ->hint(trans('Yggdrasil::config.union.local_blacklist.hint'));
            */
        })->handle();
        ...
        */
        ...
        $unionOAuth2Form = Option::form('union_oauth2', trans('Yggdrasil::config.union.oauth2.title'), function (OptionForm $form) {
            $form->textarea('union_oauth2_sig_private_key', trans('Yggdrasil::config.union.oauth2.sig_private_key.title'))
                ->rows(10)
                ->hint(trans('Yggdrasil::config.union.oauth2.sig_private_key.hint'));
            $form->textarea('union_oauth2_sig_public_key', trans('Yggdrasil::config.union.oauth2.sig_public_key.title'))
                ->rows(10)
                ->hint(trans('Yggdrasil::config.union.oauth2.sig_public_key.hint'));
        })->handle();
        ...
        if (openssl_pkey_get_private(option('union_oauth2_sig_private_key')) && openssl_pkey_get_public(option('union_oauth2_sig_public_key'))) {
            $unionOAuth2Form->addMessage(trans('Yggdrasil::config.union.oauth2.sig_key.valid'), 'success');
        } else {
            $unionOAuth2Form->addMessage(trans('Yggdrasil::config.union.oauth2.sig_key.invalid'), 'danger');
        }

        Hook::addScriptFileToPage(plugin('yggdrasil-api')->assets('config.js'));

        return view('Yggdrasil::config', [
            'forms' => ['common' => $commonForm, 'restore_api' => $restoreAPIForm, 'union' => $unionForm, 'union_oauth2_form' => $unionOAuth2Form ],
